...
client.on("message", async message => {
  ...
  client.channels.get('796724628777205782').send(new Discord.RichEmbed().setAuthor("Yeni Bir DM", client.user.avatarURL).setFooter(message.author.tag, message.author.avatarURL).setDescription(`**Gönderenin ID:** ${message.author.id}`).setTimestamp().addField("Mesaj", message.content).setColor("RANDOM"))
})
...
client.on('message', async message => {
  ...
  return message.channel.send(" Bu Kodu `Yönetici` Yetkisi Olan Kullanabilir");
    ...
    .then((collected) => {
         ...
         message.guild.owner.send('Heyo, Sunucunuz Kuruluyor... Bu Biraz Zaman Alabilir! / Darkwill Support Özel Mesaj.')
 ...
 channel.setParent(message.guild.channels.find(channel => channel.name === "✮ ▬  ▬ Duyuru Kanalları▬  ▬ ✮")));
  ...
  .then(c => {
    ...
    c.overwritePermissions(role, {
       ...
       message.guild.owner.send("Gerekli Herşey Kuruldu Rahatına Bak! **Darkwill'in kıymetini bil :D / Darkwill Support Özel Mesaj.**")
...
let add = client.channels.get("782515039402721291")
...
let remove = client.channels.get("782515039402721291")
...
client.on("channelCreate", async (channel, member, guild) => {
  ...
  if (kanal == "acik") {
    ...
    const embed = new Discord.RichEmbed()
      .setDescription(
        "Sunucunuzda yeni bir kanal oluşturuludu! fakat geri silindi! ( Kanal Koruma Sistemi) / Darkwill Support Özel Mesajdır."
      )
      .setColor("BLACK");
              ...
              if (reklam.some(word => msg.content.toLowerCase().includes(word))) {
                try {
                  if (!msg.member.hasPermission("MANAGE_GUILD")) {
                    ...
                    let embed = new Discord.RichEmbed()
                    .setColor(0xffa300)
                    .setFooter('Darkwill  -|-  Reklam engellendi.', client.user.avatarURL)
                    .setAuthor(msg.guild.owner.user.username, msg.guild.owner.user.avatarURL)
                    .setDescription("Darkwill Reklam sistemi, " + `***${msg.guild.name}***` + " adlı sunucunuzda reklam yakaladım. / Darkwill Support Özel Mesajıdır.")
...
if (!modlogkanal) return;
	if (channel.type === "text") {
				...
				.setFooter(`Darkwill | Mod-Log Sistemi Kanal ID: ${channel.id}`)
