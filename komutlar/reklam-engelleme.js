...
exports.run = async(client, message, args) => {
  ...
  if (!args[0]) return message.channel.send(`:no_entry: Reklam Filtresini Ayarlamak İçin \`s!reklam-engelleme aç\` | Kapatmak İstiyorsanız \`s!reklam-engelleme kapat\` Yazabilirsiniz`)
  if (args[0] !== 'aç' && args[0] !== 'kapat') return message.channel.send(`:no_entry: Reklam Filtresini Ayarlamak İçin \`s!reklam-engelleme aç\` | Kapatmak İstiyorsanız \`!reklam-engelleme kapat\` Yazabilirsiniz`)
